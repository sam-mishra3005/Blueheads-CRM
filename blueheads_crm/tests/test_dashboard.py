...
from blueheads_crm.api.dashboard import (
...
class TestDashboard(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.user = "blueheads_crm.manager@example.com"  # CRM manager from test_records.json
		cls.user2_email = "blueheads_crm.user1@example.com"  # Test user from test_records.json
	...
	def test_get_total_leads(self):
		...
		# Test with user filter - blueheads_crm.user1@example.com owns 3 leads
	...
	def test_get_ongoing_deals(self):
		...
		# Test with user filter - blueheads_crm.user1@example.com owns 2 ongoing deals
	...
	def test_get_won_deals(self):
		...
		# Test with user filter - blueheads_crm.user1@example.com owns 0 won deals
	...
	def test_get_average_deal_value(self):
		...
		# Test with user filter - blueheads_crm.user1@example.com owns 2 ongoing deals (no won)
	...
	def test_user_filtering_isolation(self):
		...
		self.assertEqual(result_crm_user["value"], 3)  # blueheads_crm.user1@example.com owns 3 leads
	...
	def test_user_isolation_across_multiple_metrics(self):
		...
		self.assertEqual(user_leads, 3)  # blueheads_crm.user1 owns 3 leads
		self.assertEqual(user_ongoing, 2)  # blueheads_crm.user1 owns 2 ongoing deals
		self.assertEqual(user_won, 0)  # blueheads_crm.user1 owns 0 won deals
