...
class ERPNextCRMSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from blueheads_crm.fblueheads_crm.doctype.crm_product_sync_issue.crm_product_sync_issue import CRMProductSyncIssue
	...
	def validate(self):
		...
		if self.enabled:
			...
			if not was_active and not self.is_erpnext_in_different_site:
				from blueheads_crm.fblueheads_crm.doctype.crm_product.reconcile_job import enqueue_reconciliation
	...
	def create_custom_fields_in_remote_site(self):
		...
		try:
			client.post_api("erpnext.blueheads_crm.frappe_crm_api.create_custom_fields_for_frappe_crm")
		except Exception:
			frappe.log_error(
...
def create_prospect_in_remote_site(crm_deal, erpnext_crm_settings):
	try:
		...
		return client.post_api(
			"erpnext.blueheads_crm.frappe_crm_api.create_prospect_against_crm_deal",
			{
...
def create_customer_from_deal(doc, erpnext_crm_settings):
	...
	try:
		if not erpnext_crm_settings.is_erpnext_in_different_site:
			try:
				from erpnext.blueheads_crm.frappe_crm_api import create_customer
			except ImportError:
				frappe.throw(_("ERPNext is not installed in the current site"))
			...
			customer_name = client.post_api("erpnext.blueheads_crm.frappe_crm_api.create_customer", customer_data)
...
def get_crm_form_script():
	...
	setActions() {
		...
		this.actions.push({
			...
			onClick: () => {
				call(
					"blueheads_crm.fblueheads_crm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_quotation_url",
					{
		...
		call("blueheads_crm.fblueheads_crm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_customer_link", {
			crm_deal: this.doc.name
		}).then((customer_url) => {
			if (customer_url) {
				this.actions.push({
