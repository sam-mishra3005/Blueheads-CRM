...
from blueheads_crm.fblueheads_crm.doctype.crm_deal.crm_deal import (
	add_contact,
	create_deal,
	remove_contact,
	set_primary_contact,
...
class TestCRMDeal(IntegrationTestCase):
	...
	def test_owner_cleared_on_unassign(self):
		...
		deal = create_test_deal(organization="Owner Clear Org", deal_owner="blueheads_crm.user1@example.com")
		self.assertEqual(deal.deal_owner, "blueheads_crm.user1@example.com")

		assign_remove("CRM Deal", deal.name, "blueheads_crm.user1@example.com")
	...
	def test_reassignment_moves_owner(self):
		...
		deal = create_test_deal(organization="Reassign Org", deal_owner="blueheads_crm.user1@example.com")

		# Frappe assignment rules unassign before assign in one cycle; mirror that order
		assign_remove("CRM Deal", deal.name, "blueheads_crm.user1@example.com")
		self.assertIsNone(frappe.db.get_value("CRM Deal", deal.name, "deal_owner"))

		assign_add({"assign_to": ["blueheads_crm.user2@example.com"], "doctype": "CRM Deal", "name": deal.name})

		self.assertEqual(frappe.db.get_value("CRM Deal", deal.name, "deal_owner"), "blueheads_crm.user2@example.com")

	def test_assignment_overrides_owner(self):
		"""A new assignment takes ownership even when an owner already exists (newest owns)."""
		deal = create_test_deal(organization="Override Org", deal_owner="blueheads_crm.user1@example.com")
		assign_add({"assign_to": ["blueheads_crm.user2@example.com"], "doctype": "CRM Deal", "name": deal.name})
		self.assertEqual(frappe.db.get_value("CRM Deal", deal.name, "deal_owner"), "blueheads_crm.user2@example.com")
	...
	def test_removing_any_assignee_clears_owner(self):
		...
		deal = create_test_deal(organization="Wrinkle Org", deal_owner="blueheads_crm.user1@example.com")
		assign_add({"assign_to": ["blueheads_crm.user2@example.com"], "doctype": "CRM Deal", "name": deal.name})
		# newest assignment owns
		self.assertEqual(frappe.db.get_value("CRM Deal", deal.name, "deal_owner"), "blueheads_crm.user2@example.com")

		assign_remove("CRM Deal", deal.name, "blueheads_crm.user1@example.com")  # remove a non-owner assignee
		self.assertIsNone(frappe.db.get_value("CRM Deal", deal.name, "deal_owner"))
	...
	def test_task_unassign_does_not_touch_owner(self):
		...
		assign_add({"assign_to": ["blueheads_crm.user1@example.com"], "doctype": "CRM Task", "name": task.name})
		# Should not raise
		assign_remove("CRM Task", task.name, "blueheads_crm.user1@example.com")
