...
from blueheads_crm.fblueheads_crm.doctype.crm_lead.crm_lead import convert_to_deal
...
class TestCRMLead(IntegrationTestCase):
	...
	def test_lead_owner_cannot_be_same_as_email(self):
		...
		with self.assertRaises(frappe.exceptions.ValidationError) as context:
			create_lead(
				first_name="Test",
				email="blueheads_crm.user1@example.com",
				lead_owner="blueheads_crm.user1@example.com",
			)
	...
	def test_update_lead_owner(self):
		...
		lead.lead_owner = "blueheads_crm.user1@example.com"
		lead.save()
		lead.reload()

		# Verify new owner is assigned and shared
		self.assertEqual(lead.lead_owner, "blueheads_crm.user1@example.com")
		new_docshare = frappe.db.exists(
			"DocShare",
			{"user": "blueheads_crm.user1@example.com", "share_name": lead.name, "share_doctype": "CRM Lead"},
		)
	...
	def test_assignees_transferred_on_conversion(self):
		...
		lead.assign_agent("blueheads_crm.user1@example.com")

		lead_assignees = lead.get_assigned_users()

		self.assertIn("Administrator", lead_assignees)
		self.assertIn("blueheads_crm.user1@example.com", lead_assignees)
		...
		self.assertIn("blueheads_crm.user1@example.com", deal_assignees)

	def test_owner_cleared_on_unassign(self):
		"""Unassigning the current owner clears lead_owner"""
		lead = create_lead(first_name="Owner", lead_owner="blueheads_crm.user1@example.com")
		self.assertEqual(lead.lead_owner, "blueheads_crm.user1@example.com")

		assign_remove("CRM Lead", lead.name, "blueheads_crm.user1@example.com")
	...
	def test_assignment_overrides_owner(self):
		...
		lead = create_lead(first_name="Override", lead_owner="blueheads_crm.user1@example.com")
		assign_add({"assign_to": ["blueheads_crm.user2@example.com"], "doctype": "CRM Lead", "name": lead.name})
		self.assertEqual(frappe.db.get_value("CRM Lead", lead.name, "lead_owner"), "blueheads_crm.user2@example.com")
